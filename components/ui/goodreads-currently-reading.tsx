...
export const Component = ({ goodreadsUserId = "117356165-mohammad-abbas" }) => {
  ...
  const parseRSSItem = (item: any) => {
    // Direct tag extraction - much cleaner!
    const title = item.querySelector("title")?.textContent?.trim() || "Unknown Title"
    const author = item.querySelector("author_name")?.textContent?.trim() || "Unknown Author"
    ...
    const largeImage = item.querySelector("book_large_image_url")?.textContent?.trim()
    const mediumImage = item.querySelector("book_medium_image_url")?.textContent?.trim()
    const smallImage = item.querySelector("book_small_image_url")?.textContent?.trim()

    // Use the best available image
    const imageUrl = largeImage || mediumImage || smallImage

    // Construct Goodreads book URL
    const bookUrl = bookId ? `https://www.goodreads.com/book/show/${bookId}` : null
    ...
    <div className="space-y-12 p-12">
      ...
      <div className="flex items-start gap-12">
        {book.imageUrl && book.bookUrl ? (
          <a href={book.bookUrl} target="_blank" rel="noopener noreferrer" className="flex-shrink-0">
            <img
        ...
        <div className="flex-1 space-y-6 min-w-0">
          {book.bookUrl ? (
            ...
            <h4 className="text-5xl font-black text-foreground leading-tight line-clamp-4">{book.title}</h4>
          )}

          <p className="text-3xl text-muted-foreground font-bold">by {book.author}</p>
          ...
          <div className="flex items-center gap-12 text-2xl text-muted-foreground font-semibold">
            ...
            {book.published && <div className="text-xl opacity-75">Published {book.published}</div>}
          </div>
        </div>
      </div>
    </div>
