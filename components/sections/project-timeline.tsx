  ...
  return (
    <section id="project-timeline">
      {steps.map((s) => (
        <div
          key={s.id}
          className="flex flex-col lg:flex-row justify-between gap-[4vw] px-[4vw] py-[4vw]"
          style={{ backgroundColor: s.bg, color: s.color }}
        >
          {/* Step number */}
          <div className="flex-shrink-0">
            <p
              className="text-[clamp(4rem,10vw,10rem)] font-bold leading-none"
              style={{ fontFamily: "'Fira Code', monospace", opacity: 0.15 }}
            >
              {s.id}
            </p>
          </div>
          ...
          <div className="flex-1 max-w-2xl flex flex-col justify-center">
            <hr className="border-t mb-4" style={{ borderColor: s.color === "#fff" ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)" }} />
            <h3
              className="text-[clamp(1.5rem,4vw,4rem)] font-bold leading-[0.9] uppercase tracking-tight mb-4"
              style={{ fontFamily: "'Fira Code', monospace" }}
            >
              {s.title}
            </h3>
            <p className="text-[clamp(0.9rem,1.5vw,1.1rem)] leading-relaxed" style={{ opacity: 0.7 }}>
              {s.desc}
            </p>
          </div>
        </div>
      ...
      >
        ...
        >
          Ready To
          <br />
          <span style={{ color: "#C3E41D" }}>Start?</span>
        </h3>
        <hr className="border-t border-white/10 mb-[3vw]" />
        <div className="flex flex-wrap gap-6">
          ...
          <Link
            href="/portfolio"
            className="inline-flex items-center gap-3 text-[clamp(0.9rem,1.5vw,1.1rem)] font-bold uppercase tracking-widest text-white/50 hover:text-white transition-colors duration-300"
          >
            <span className="flex items-center justify-center w-10 h-10 rounded-full border-2 border-white/30">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M5 12h14M12 5l7 7-7 7" />
              </svg>
            </span>
